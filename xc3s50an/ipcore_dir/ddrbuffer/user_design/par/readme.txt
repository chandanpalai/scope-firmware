...
* After integrating the design with user's testbench, the user needs to add
  these files (testbench files, toplevel wrapper file) in ddrbuffer.prj under
  synth folder.
...
* ddrbuffer.ucf file is the constraint file for the design. This is used
...
* "icon_coregen.xco", "ila_coregen.xco" and "vio_coregen.xco" files are
  ...
  ChipScope modules i.e., ila and icon in the ddrbuffer rtl file.
...
Note : When you generate the design using "Debug Signals for Memory Controller"
       ...
       into ise_flow.bat and create_ise.bat files. The ddrbuffer rtl file
...
* At the start of a Chip Scope Analyzer project, all of the signals in
  every core have generic names. "ddrbuffer.cdc" is a file that contains
  all the signal names of all cores. Upon importing this file, signal names are
  renamed to the specified names in "ddrbuffer.cdc" file. This file will work
...
* For example, the design is generated with component name as ddrbuffer with
  Target FPGA 4000FG1156 and Compatible FPGA 5000FG1156.
   ...
   - Change the UCF file name in batch file to the one in compatible_ucf
     folder i.e., change the UCF name (ddrbuffer.ucf) in ise_flow.bat and
